...
is_community: false
---
このSNSの展開にはいくつかの機能が含まれます：**1.**システム機能として提供されるSNS（SNSサブネット上に配置され、保守可能なアップグレードを容易にする）、**2.**将来的にさらにカスタマイズ可能なSNSの投票報酬の最初のバージョン、**3.**参加者がSNSトークンと引き換えにICPトークンを提供する、dapp を分散化する分散化スワップ。**4.**エンドユーザーが分散化スワップに参加できるNNSフロントエンドdapp 拡張。**5.**ユーザーがSNSを初期化するためのツール。
