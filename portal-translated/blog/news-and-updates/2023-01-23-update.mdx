...
、常にunitで評価されるように強制され るため、あまり便利ではありません。
...
もnullになります。し か し 、`x` に Nat 値があ っ た と き は、 その値は 3 に加算 さ れてオプシ ョ ン型に包まれ、
から`y` へ代入 さ れます！

こ れは、
オプシ ョ ン型の中間値を含む可能性のある副次効果を実行するために、`ignore` キーワー ド と 組み合わせて も 有用です。
