...
言語チームの @matthewhammer はMotoko の安定領域と呼ばれる新機能に懸命に取り組んでいます。このアイデア（以前はページアロケータと呼ばれていました）の背景にあるのは、ユーザーが互いに隔離された安定したメモリの領域へのアクセスをリクエストできるようにすることです。この機能は確かに多くのユースケースを解き放つでしょうが、これによる明らかな影響のひとつは、安定したメモリにデータを格納するMotoko で安定したデータ構造を作成できるようになることです（安定したマップ、バッファなどを考えてみてください）。
