...
ユーザが自分自身や使用しているウォレットを削除すると、canister の制御を失う可能性があります。 この損失を防ぐために、`dfx canister update-settings` は、プリンシパルまたはウォレットがcanister のコントローラリストから削除されたときに、確認を求めるようになりました。 この確認は、`–yes`/`-y` 引数を使用するか、次のようなパイプを使用してコマンドに 'yes' を渡すことで、スキップできます：
