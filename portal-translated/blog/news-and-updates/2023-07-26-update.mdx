...
Runtime チームの Ulan Degenbaev 氏のおかげで、新しい Rust サンプルが`DFINITY/examples` リポジトリに追加されました。このサンプルは、IC上のdapps 、画像処理のような長時間実行する計算タスクを単一のメッセージ実行で実行する方法を示しています。この機能は、Deterministic Time Slicing (DTS)によって実現されています。DTSは、長い計算タスクをより小さなスライスに分割し、複数のブロックにわたって実行するIC独自の機能です。この新機能を使用することで、開発者は通常と同じように長時間実行する計算コードを書くことができ、DTSの利点を活用するために特別なタスクを実行する必要はありません。
...
`dfx deploy --playground` は、まもなくdfxで利用可能になる予定の機能です。この新しい フラグを使うと、 を プレイグラウンドに即座にデプロイすることができます。このフラグを使用すると、dfx プロジェクト内の が IC メインネットにデプロイされ、 の URL が開発者に返されます。このデプロイ方法によって、開発者は最初に ウォレットをセットアップしたり を取得したりすることなく、 をメインネットに素早くデプロイできます。これにより、開発者はすぐに dfx を使い始めることができ、開発者のオンボーディングプロセスを支援することができます。`--playground` canisters Motoko canisters canister cycles cycles canisters 
