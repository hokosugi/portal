...
`~/.cache/dfinity/versions` ディレクトリには、SDK のバージョン管理されたサブディレクトリが 1 つ以上格納されています。各バージョン別サブディレクトリには、SDK の特定のバージョンに必要なすべてのディレクトリとファイルが含まれます。たとえば、`~/.cache/dfinity/versions/0.9.3` ディレクトリの内容をリストアップすると、次のコアコンポーネントが表示されます：
